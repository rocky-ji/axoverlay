 ...
 * @param is_mirrored TRUE if mirroring is enabled for the stream.
 * @return Returns TRUE if overlays should be rendered for the stream.
 */
static gboolean
select_callback(gint camera, gint width,
			gint height, gint rotation,
			gboolean is_mirrored,
			enum axoverlay_stream_type type)
{
	return TRUE;
	//overlay on camera 1 and 1280x720 resolution. Rocky Ji
	if (camera == 1)
	{
...
static void draw_text(cairo_t* context, const gint pos_x, const gint pos_y) {
   ...
   //font is under /usr/share/fonts/truetype/wqy  Chinese
	cairo_select_font_face(context, "WenQuanYi Micro Hei", CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_BOLD);
    ...
    str = g_strdup_printf("Countdown测试简体繁體寫法 %i", counter);
...
int main(void) {
	...
	//main函数里面要指定一下这个回调函数
	settings.select_callback=select_callback;//setup here to only display overlay on selected resolution and camera channel
